...
class Sekolah extends MY_Controller {
   ...
   public function website_check($str) {
      if (strpos($str, '.com')) {
         return TRUE;
      } elseif (strpos($str, '.co.id')) {
         return TRUE;
      } elseif (strpos($str, '.sch.id')) {
         return TRUE;
      } else {
         $this->form_validation->set_message('website_check', 'Aplikasi ini khusus untuk domain resmi sekolah. Silahkan masukan nama domain resmi sekolah! contoh : http://www.namasekolah.sch.id, bisa juga menggunakan domain .com dan .co.id');
         return FALSE;
      }
   }
}
